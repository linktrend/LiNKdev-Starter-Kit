# Usage Metering Implementation Guide
...
For metered billing (e.g., charge per API call over limit):

```typescript
// Calculate overage charges
async function calculateOverageCharges(
  orgId: string,
  month: Date
) {
  const { data: subscription } = await supabase
  ...
  const usage = await getMonthlyUsageFromAggregations(
    orgId,
    month.getFullYear(),
    month.getMonth() + 1
  );
  ...
  if (usage.api_call > limits.max_api_calls_per_month) {
    const overage = usage.api_call - limits.max_api_calls_per_month;
    const charge = overage * 0.001; // $0.001 per extra call
    overages.push({
      metric: 'api_call',
      overage,
      charge
  ...
  if (storageGB > limits.max_storage_gb) {
    const overage = storageGB - limits.max_storage_gb;
    const charge = overage * 0.10; // $0.10 per extra GB
    overages.push({
      metric: 'storage',
      overage,
      charge
...
// Create Stripe invoice for overages
async function createOverageInvoice(orgId: string, month: Date) {
  const overages = await calculateOverageCharges(orgId, month);
  ...
  for (const overage of overages) {
    await stripe.invoiceItems.create({
      customer: customer.stripe_customer_id,
      invoice: invoice.id,
      amount: Math.round(overage.charge * 100), // cents
      currency: 'usd',
      description: `${overage.metric} overage: ${overage.overage.toFixed(2)} units`
    });
  }
...
### 2. Cache Limits in Memory

```typescript
// Cache plan limits to avoid repeated queries
const limitsCache = new Map<string, Record<string, number>>();

async function getOrgLimitsWithCache(orgId: string) {
  if (limitsCache.has(orgId)) {
    return limitsCache.get(orgId);
  }
  
  const limits = await getOrgLimits(orgId);
  limitsCache.set(orgId, limits);
  
  // Expire after 5 minutes
  setTimeout(() => limitsCache.delete(orgId), 5 * 60 * 1000);
...
1. **Implement Event Logging** - Add logging to all relevant actions
2. **Set Up Cron Jobs** - Configure daily/monthly aggregation
3. **Build Usage Dashboard** - Create UI to display usage
4. **Add Limit Enforcement** - Implement middleware to block over-limit actions
5. **Test Thoroughly** - Ensure accurate tracking and enforcement
6. **Monitor Performance** - Watch for slow queries, optimize as needed

---

**End of Usage Metering Implementation Guide**
