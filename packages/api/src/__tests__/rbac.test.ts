...
describe('RBAC Utilities', () => {
  describe('roleIsSufficient', () => {
    ...
    it('should return false when user role is insufficient', () => {
      ...
      expect(roleIsSufficient('admin', 'viewer')).toBe(false);
  ...
  describe('isRoleHigher', () => {
    ...
    it('should return false for equal or lower roles', () => {
      ...
      expect(isRoleHigher('editor', 'admin')).toBe(false);
      expect(isRoleHigher('viewer', 'editor')).toBe(false);
    });
  });
  ...
  describe('canChangeRole', () => {
    it('should allow owner to change any role except owner', () => {
      expect(canChangeRole('admin', 'editor', 'owner')).toBe(true);
      expect(canChangeRole('editor', 'admin', 'owner')).toBe(true);
      expect(canChangeRole('viewer', 'editor', 'owner')).toBe(true);
      expect(canChangeRole('owner', 'admin', 'owner')).toBe(false);
    });

    it('should allow admin to change editor/viewer roles', () => {
    ...
    it('should not allow editor/viewer to change roles', () => {
      expect(canChangeRole('viewer', 'editor', 'editor')).toBe(false);
      expect(canChangeRole('viewer', 'editor', 'viewer')).toBe(false);
    });

    it('should not allow promoting to owner unless you are owner', () => {
      expect(canChangeRole('admin', 'owner', 'admin')).toBe(false);
      expect(canChangeRole('editor', 'owner', 'owner')).toBe(false);
    });

    it('should return false for null actor role', () => {
      expect(canChangeRole('viewer', 'editor', null)).toBe(false);
    });
  });
  ...
  describe('Role Permissions Constants', () => {
    it('should have correct permissions for each role', () => {
      ...
      expect(ROLE_PERMISSIONS.editor).toContain('view_content');
      expect(ROLE_PERMISSIONS.editor).not.toContain('manage_members');
